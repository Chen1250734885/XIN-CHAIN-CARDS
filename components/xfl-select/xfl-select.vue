<template>
	<div class="show-box"  :class="{disabled: disabled, active: isShowList}" :style="style_Container">
		...
		<!-- 输入框，仅在可输入模式下使用 -->
		<input  
			v-if="showInput"  class="input"  style="display: inline-block;" placeholder-style="color: #bbb;"
			type="text" v-model="selectText"  :placeholder="placeholder"
			@focus="onFocus" @blur="onBlur" @input="onInput" @confirm="$emit('confirm', $event)"
		>
		<!-- 显示框 -->
		<div v-else class="input" style="display: inline-block;" :class="{placeholder: selectText === placeholder}" @click="onUpperClick" v-for="(item, index) in innerList" :key="index"  >{{item.value}}</div>
		...
		:style="'top:' + listTop__ + 'px;'" v-show="isShowList">
			...
			<scroll-view 
				class="list" style="box-shadow: 0px 8px 29px 3px rgba(25, 25, 25, 0.27);z-index: 9999;background-color: #fff;padding: 0 10rpx;"  
...
<style scoped >
	...
	.show-box{
		...
		/****** 列表框部分样式 *****/
		.list-container{
			position: absolute; width: 100%; left: 2rpx; top: 50rpx;
			box-sizing: border-box; z-index: 100;background-color: #666666;
