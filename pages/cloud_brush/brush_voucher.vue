		...
		<view slot="gBody">
			...
			<view class="xzshiyong">
				<view style="display: flex;justify-content: space-between;">
					...
					<view style="transform: translateY(36rpx);margin-left: 52rpx;">
						<text style="font-size: 36rpx;">余额：{{balance}}.00</text>
					</view>
					<view style="transform: translateY(31rpx);margin-left: 52rpx;margin-right: 51rpx;">
						<text style="font-size: 30rpx;">还款手续费：{{shoyxufei}}</text>
					</view>
				</view>
				...
				<view style="display: flex;justify-content: space-between;">
					<view class="shiyan" @tap="shiys" :style="{background:wxzhongs==0?'#DBDBDB':'#DBDBDB'}">
						...
						<text style="margin-left: 75rpx;" :style="{color:wxzhongs==0?'':''}">确认使用</text>
					</view>
					<view class="bushiyan" @tap="bushiyong" :style="{background:xzhongs==0?'#DBDBDB':'#DBDBDB'}">
						...
						<text style="margin-left: 50rpx;" :style="{color:xzhongs==0?'':''}">不使用抵用券</text>
					</view>
				</view>
				<!-- 生成计划按钮 -->
			...
			<view>
				...
				<!-- 券列表 -->
				<!-- 可分享的赠送券 -->
				<view style="margin-left: 23rpx;" v-if="zengs.show_num==0" @tap="fanx">
					<image :src="zengs.img_url" mode="" style="width: 696rpx;height: 216rpx;"></image>
				</view>
				<!-- 可分享的赠送券 -->
				<view style="margin-left: 23rpx;"v-else>
					<image :src="zengs.img_url" mode="" style="width: 696rpx;height: 216rpx;"></image>
					<text class="shenyu">剩余抵扣券：{{zengs.show_num}}</text>
					<!-- 充值按钮 -->
					<view class="chozhi" @tap="chongz(zengs.type,zengs.id,zengs.show_num)" :style="{background:zengs.show_num==0?'#DBDBDB':''}">
						<text :style="{color:zengs.show_num==0?'#000000':''}">充值</text>
					</view>
					<!-- 转让按钮 -->
					<view  class="zhuanras" @tap="zhuanrangs(zengs.type,zengs.id,zengs.show_num)" :style="{background:zengs.show_num==0?'#DBDBDB':''}">
			...
			<graceShade background="rgba(0,0,0,0.8)" :show="show" style="z-index: 999;">
				<view :style="{backgroundImage: 'url('+Recharge+')'}" style="width: 750rpx;height: 1225rpx; background-size: cover; ">
			...
			<graceDialog :isTitle="false" :show="show1" :isCloseBtn="false" width='500rpx'>
				...
				<view slot="btns" class="grace-space-between">
					<text class="grace-dialog-buttons" @tap="closeDialog1">关闭</text>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 鉴权
			jianquan() {
				var shuju = uni.getStorageSync('storage')
				var vm = this;
				console.log({
					card: this.card_id
				})
				vm.req.post(
					...
					function(res) {
						...
						if (is_need_SMS === 0) {
							vm.msg_show(res.msg);
							vm.Goto('/pages/preview_plan/preview_plan?channel_id=' + shuju.channel_id + '&bc_id=' + shuju.card_id +
								'&times=' +
								shuju.bishu + '&dates=' + shuju.date + '&pay_money=' + shuju.vale + '&had_money=' + shuju.zhouzhuanjine +
								'&c_code=' + shuju.c_code + '&now_time=' + shuju.now_time + '&schema=' + shuju.schema + '&coupon=' + vm.coupons
							);
							...
							vm.Goto('/pages/web_view/web_view?url=' + res.data.url + '&channel_id=' + shuju.channel_id + '&bc_id=' + shuju.card_id +
								'&times=' +
								shuju.bishu + '&dates=' + shuju.date + '&pay_money=' + shuju.vale + '&had_money=' + shuju.zhouzhuanjine +
								'&c_code=' + shuju.c_code + '&now_time=' + shuju.now_time + '&schema=' + shuju.schema + '&coupon=' + vm.coupons
							);
					...
					vm.req.post(
						...
						function(res) {
							if (res.code == 200) {
								...
								vm.Goto('/pages/preview_plan/preview_plan?channel_id=' + shuju.channel_id + '&bc_id=' + shuju.card_id +
									'&times=' + shuju.bishu + '&dates=' + shuju.date + '&pay_money=' + shuju.vale + '&had_money=' + shuju.zhouzhuanjine +
									'&c_code=' + shuju.c_code + '&now_time=' + shuju.now_time + '&schema=' + shuju.schema + '&coupon=' + vm.coupons
								);
							} else {
								vm.msg_show(res.msg);
			...
			generate_plan() {
				var shuju = uni.getStorageSync('storage')
				console.log(shuju);
				this.zhouzhuanjine = shuju.zhouzhuanjine
				this.card_name = shuju.card_name
				var vm = this;
				vm.$refs.popup.open()
			},
			// 生成计划
						...
						if (res.data.authentication == 1) {
							vm.req.mypost(
								vm.lochost + '/index/index?method=make_refund_plan', {
									...
									times: shuju.bishu,
									dates: shuju.date,
									pay_money: shuju.vale,
									had_money: shuju.zhouzhuanjine,
									c_code: shuju.c_code, //城市编码
									now_time: shuju.now_time,
									schema: shuju.is_default,
								},
								function(res) {
									console.log(res);
									if (res.code == 200) {
